 ...
 */
#define S_FUNCTION_NAME                sfunar_powerSTEP01_releasesw
...
static void mdlCheckParameters(SimStruct *S)
{
  ...
  if EDIT_OK(S, 0) {
	...
	if EDIT_OK(S, 5) {
	  ...
	  /* Check the parameter attributes */
	  ssCheckSFcnParamValueAttribs(S, 5, "action", DYNAMICALLY_TYPED, 2, dimsArray, 0);
	}
...
{    
  ...
  ssSetSFcnParamTunable(S, 5, 0);	// action
  ssSetNumPWork(S, 0);
...
static void mdlSetWorkWidths(SimStruct *S)
{
  ...
  ssRegDlgParamAsRunTimeParam(S, 5, 5, "action", SS_UINT8);
}
