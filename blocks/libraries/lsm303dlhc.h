...
#define LSM303DLHC_ACC_BASE_ADDR 0x32
#define LSM303DLHC_MAG_BASE_ADDR 0x3C
...
#define LSM303DLHC_ODR_PWR_DOWN						 ((uint8_t)0x00)   
...
#define LSM303DLHC_HPFCF_OFF							 ((uint8_t)0x00)
#define LSM303DLHC_HPFCF_8                 ((uint8_t)0x08)
#define LSM303DLHC_HPFCF_16                ((uint8_t)0x18)
#define LSM303DLHC_HPFCF_32                ((uint8_t)0x28)
#define LSM303DLHC_HPFCF_64                ((uint8_t)0x38)
...
private:
	...
	float sensitivity_mag_xy;
	float sensitivity_mag_z;
  ...
  void write_i2c(uint8_t reg, uint8_t *data, uint8_t len);
	...
	void get_xyz_raw(uint8_t reg, int16_t *x, int16_t *y, int16_t *z);

	int16_t get_temp_raw(void);

public:
	// constructor
	LSM303DLHC(I2C *i2c);

	bool init_acc(uint8_t rate, uint8_t scale, uint8_t hp_filter, bool drdy_int);

	bool init_mag(uint8_t rate, uint8_t scale, uint8_t enable_temp);

	void get_xyz_acc(float *x, float *y, float *z);

	void get_xyz_mag(float *x, float *y, float *z);
