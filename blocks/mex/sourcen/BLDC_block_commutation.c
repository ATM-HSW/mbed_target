...
/*
 *   Block_commutation.c Simple C-MEX S-function for function call.
 *
 */
 ...
 */
#define S_FUNCTION_NAME                BLDC_block_commutation
...
static void mdlInitializeSizes(SimStruct *S)
{
   ...
   */
  if(mxGetScalar(ssGetSFcnParam(S, 2))>1)numInput++;
  if(mxGetScalar(ssGetSFcnParam(S, 1)))numInput++;

  if (!ssSetNumInputPorts(S, numInput))
    return;
...
static void mdlSetWorkWidths(SimStruct *S)
{
   ...
   */
  ssRegDlgParamAsRunTimeParam(S, 1, 0, "power", ssGetDataTypeId(S, "uint8"));
  /*
   * Register the run-time parameter 3
   */
  ssRegDlgParamAsRunTimeParam(S, 2, 1, "mode", ssGetDataTypeId(S, "uint8"));
