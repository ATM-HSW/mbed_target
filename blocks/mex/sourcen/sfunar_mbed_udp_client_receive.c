...
#define S_FUNCTION_NAME  sfunar_mbed_udp_client_receive
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    // Now we need two now how many Outputports we must set
    
    nOutputPorts  = mxGetNumberOfElements(ssGetSFcnParam(S,1));
    /*
    ...
    for(i = 0; i< nOutputPorts; i++)
    {
		...
		if (strstr(mxArrayToString(mxGetCell(ssGetSFcnParam(S, 1), i)), "uint8"))
			ssSetOutputPortDataType(S, i, 3);
		else if (strstr(mxArrayToString(mxGetCell(ssGetSFcnParam(S, 1), i)), "uint16"))
			ssSetOutputPortDataType(S, i, 5);
		else if (strstr(mxArrayToString(mxGetCell(ssGetSFcnParam(S, 1), i)), "uint32"))
			ssSetOutputPortDataType(S, i, 7);
		else
...
#define MDL_RTW  /* Change to #undef to remove function */
  ...
  real_T  nBufferBytes = 0;
	...
	for (i = 0; i < mxGetNumberOfElements(ssGetSFcnParam(S, 1)); i++)
	{
		if (strstr(mxArrayToString(mxGetCell(ssGetSFcnParam(S, 1), i)), "uint8"))
			nBufferBytes++;
		else if (strstr(mxArrayToString(mxGetCell(ssGetSFcnParam(S, 1), i)), "uint16"))
